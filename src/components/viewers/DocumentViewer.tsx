...
/**
 * Renders office documents (DOCX, XLSX, PPTX, RTF) inline via
 * Microsoft Office Online viewer. Requires a publicly-accessible
...
export function DocumentViewer({ url, fileName }: DocumentViewerProps) {
  ...
  // Microsoft Office Online viewer can render DOCX, XLSX, PPTX, etc.
  ...
  if (hasError) {
    return (
      <div className="flex h-full flex-col items-center justify-center px-8 text-center">
        <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-blue-50 dark:bg-blue-900/20">
        ...
        <p className="mt-1.5 max-w-xs text-xs leading-relaxed text-surface-400 dark:text-surface-500">
          This document couldn't be rendered inline. Try downloading it instead.
        </p>
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* Document rendered via Office Online */}
      <div className="relative flex-1">
        {isLoading && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 bg-surface-50 dark:bg-surface-900">
            ...
            <span className="text-xs text-surface-400">Loading document...</span>
          </div>
        )}
