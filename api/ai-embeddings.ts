 ...
 *
 * Generates text embeddings using OpenAI text-embedding-3-small (1536 dimensions)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Truncate inputs to safe token limit (~8K tokens)
    const truncated = inputTexts.map((t) => t.substring(0, 8000));
...
export const config = {
  maxDuration: 30, // Embeddings should be fast
};
